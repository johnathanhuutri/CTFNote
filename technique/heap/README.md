...
- For libc > 2.31, there is a xor mechanism added to change the behaviour of `Fd pointer`:
...
`pos` is the address of current chunk is freeing and `ptr` is the address of previous freed chunk has the same size with chunk at `pos`. For a better view, here is an example:
